...
const OrderDetails = ({ selectedOrder, userDetails }) => {
  ...
  const handleStatusChange = async (e) => {
    ...
    try {
      ...
      await axios.patch(`/api/order/${selectedOrder._id}`, { status: newStatus });
      ...
      {selectedOrder && (
        <div>
          <div className="flex flex-col mt-5">
            <div className="flex  rounded-lg bg-white flex-row relative">
...
const TableThree = () => {
  ...
  return (
    <>
            ...
            <tbody>
              {orderData.map((order, index) => (
                order.orders.map((item, key) => (
                  <tr key={key} onClick={() => openOrderSummary(item, order)}>
      ...
      <Drawer open={orderSummary} onClose={() => setOrderSummary(false)} direction="right" ease="ease-in-out" style={{ width: "33vw", zIndex: 1000000 }}>
